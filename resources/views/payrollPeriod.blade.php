                            ...
                            @foreach ($payrolls as $item)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="p-3 text-center">
                                        @if($item->status == 'paid')
                                            <span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-green-700 bg-green-50 rounded-full border border-green-200">
                                                <span class="w-1.5 h-1.5 rounded-full bg-green-600"></span> Paid
                                            </span>
                                        @else
                                            <span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-yellow-700 bg-yellow-50 rounded-full border border-yellow-200">
                                    ...
                                    <td class="p-3 text-center">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <!-- Delete Single -->
                                            <form action="{{ route('delpayroll', $item->id) }}" method="POST" class="delete-single-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition" title="Delete this slip only">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <script>
        $(document).ready(function() {
            ...
            new DataTable('#employeeTable', {
                responsive: true,
                columnDefs: [
                    { orderable: false, targets: -1 } // Disable sort kolom Action
                ]
            });
